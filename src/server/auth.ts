    ...
    session: {
      create: {
        after: async ({ data, ctx }: any) => {
          const ip = (ctx?.request?.headers?.get?.('x-forwarded-for') as string) ||
                     (ctx?.request?.headers?.get?.('x-real-ip') as string) ||
                     'unknown';
          console.log(`[AUTH] Session created for user ${(data as any)?.userId} from IP: ${ip}`);
        },
      },
      delete: {
      ...
      update: {
        after: async ({ data, oldData }: any) => {
          if ((oldData as any)?.email !== (data as any)?.email) {
            console.log(`[AUTH] Email changed for user ${(data as any)?.id}: ${(oldData as any)?.email} → ${(data as any)?.email}`);
          }
        },
      },
    },
